...
public class SimpleBenchmark {
    ...
    private final SortedSet<Account> cSeries = new BstSet<>(Account.byPrice);
    private final SortedSet<Account> cSeries2 = new BstSetIterative<>(Account.byPrice);
    private final SortedSet<Account> cSeries3 = new AvlSet<>(Account.byPrice);
    ...
    private void benchmark() throws InterruptedException {
        try {
            for (int k : COUNTS) {
                Account[] cars = new AccountGenerator().generateShuffle(k, 1.0);
                cSeries.clear();
                cSeries2.clear();
                cSeries3.clear();
            ...
            if (e.getCode() >= 0 && e.getCode() <= 3) {
                ...
                timeKeeper.logResult(MESSAGES.getString("entireSetIsPrinted"));
            } else {
