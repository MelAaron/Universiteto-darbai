...
public class BstSet<E extends Comparable<E>> implements SortedSet<E>, Cloneable {
    ...
    public Set<E> headSet(E element) {
        ...
        if (contains(element)) {
            BstNode<E> n = root;
            headRecursive(hs, n, element, false);
        }
        return hs;
...
//        throw new UnsupportedOperationException("Studentams reikia realizuoti headSet()");
    ...
    public Set<E> subSet(E element1, E element2) {
        ...
        if (contains(element1) && contains(element2)) {
            ...
            headRecursive(ss, n, element2, false);
        }
        return ss;
...
//    private void tailRecursive(Set<E> hs, BstNode<E> node) 
    ...
    private BstNode<E> headRecursive(Set<E> Set, BstNode<E> n, E d, boolean inclusive) {
        if (n == null) {
            return null;
        }
//        if (n.element != d) {
...
//        if (!n.element.equals(d)) {
            Set.add(n.element);
            headRecursive(Set, n.left, d, inclusive);
            headRecursive(Set, n.right, d, inclusive);
        } else if (inclusive) {
            Set.add(n.element);
        }
    ...
    public SortedSet<E> headSet(E toElement, boolean inclusive) {
        ...
        if (contains(toElement)) {
            BstNode<E> n = root;
            headRecursive(hs, n, toElement, false);
        }
        return hs;
    }
    ...
    public E higher(E e) {
        ...
        for (Object test : array) {
            int cmp1 = c.compare(e, (E) test);
            int cmp2 = c.compare(ats, (E) test);
    ...
    public int findHeight(BstNode<E> node) {
        BstNode<E> n = node;
        if (root == null) {
            return 0;
        }
        ...
        if (n.left != null) {
            hLeft = findHeight(n.left);
        }

        if (n.right != null) {
            hRight = findHeight(n.right);
        }
