...
import edu.ktu.ds.lab3.svencionis.*;
import edu.ktu.ds.lab3.utils.EvaluableMap;
//import edu.ktu.ds.lab3.demo.Phone;
//import edu.ktu.ds.lab3.demo.PhonesGenerator;
import edu.ktu.ds.lab3.svencionis.SimpleBenchmark;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private void initComponents() {
        ...
        paneButtons = new Panels(
                new String[]{
                    ...
                    MESSAGES.getString("button8")}, 1, 8);
    ...
    private void mapReplaceAll(){
        //KsGui.oun(taEvents,map.replaceAll(map.get(taInput.getText()), phonesGenerator.getPhone()), MESSAGES.getString("containsValue"));
        Phone ph = phonesGenerator.getPhone();
        KsGui.oun(taEvents, map.get(taInput.getText()), "Pakeistasis");
        KsGui.oun(taEvents, ph, "Pakeiciantysis");
        map.replaceAll(map.get(taInput.getText()),ph);
        //nuo cia nzn ar gerai. bet reikia paupdatint map vaizda po replaceAll
...
//        updateHashtableParameters(true);
    ...
    private void mapReplace(){
        Phone ph = phonesGenerator.getPhone();
        Phone og = map.get(taInput.getText());
        if(!map.replace(taInput.getText(), map.get(taInput.getText()) /*phonesGenerator.getPhone()*/, ph))
            KsGui.oun(taEvents,  map.replace(taInput.getText(), map.get(taInput.getText()), ph), MESSAGES.getString("replace1"));
        else {
            KsGui.oun(taEvents, MESSAGES.getString("replace"));
            KsGui.oun(taEvents, og, "Pakeistasis");
            KsGui.oun(taEvents, ph, "Pakeiciantysis");
        ...
        //map.replace(taInput.getText(), map.get(taInput.getText()), phonesGenerator.getPhone());
    }
    
    private void mapNumberOfEmpties(){
        KsGui.oun(taEvents, map.numberOfEmpties(), MESSAGES.getString("numberOfEmpties"));
        
    }
    
    private void mapPutIfAbsent(){
        if(map.putIfAbsent(taInput.getText(), map.get(taInput.getText())) == null)
            KsGui.oun(taEvents, map.get(taInput.getText()), MESSAGES.getString("putIfAbsent1"));
        else
            KsGui.oun(taEvents, map.putIfAbsent(taInput.getText(), map.get(taInput.getText())), MESSAGES.getString("putIfAbsent"));
        //map.putIfAbsent(taInput.getText(), map.get(taInput.getText()));
    }
    
    private void mapContainsValue(){
        KsGui.oun(taEvents, map.containsValue(map.get(taInput.getText())), MESSAGES.getString("containsValue"));
        if(map.containsValue(map.get(taInput.getText())))
            KsGui.oun(taEvents, map.get(taInput.getText()), "Rastas objektas");
    }
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Phone[] phonesArray = phonesGenerator.generateShufflePhonesAndIds(sizeOfGenSet, sizeOfInitialSubSet);
            for (Phone c : phonesArray) {
                map.put(
    ...
    private void mapPut() {
        Phone phone = phonesGenerator.getPhone();
        map.put(
                phonesGenerator.getPhoneId(), // Raktas
                phone);
        table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
        ...
        table.setItems(FXCollections.observableArrayList(modelList));
        updateHashtableParameters(true);
        KsGui.oun(taEvents, phone, MESSAGES.getString("mapPut"));
    }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                ...
                break;
            case 2:
                map = new ParsableHashMapOa<>(String::new, Phone::new, initialCapacity, loadFactor, ht);
                break;
