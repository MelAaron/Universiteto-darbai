...
//=================================================    
    ...
    Image deathstar = new Image( "images\\deathstar.png" );
    ImageView earthView = new ImageView(earth);
    ImageView deathstarView = new ImageView(deathstar);
    ...
    void createSpace(){
        ...
        final double radiusD = canvasW / 4 - deathstar.getWidth();
        ...
        nodes.add(deathstarView);
        deathstarView.setSmooth(false);
        deathstarView.setScaleX(1.5);
        deathstarView.setScaleY(1.5);
        final long startNanoTime = System.nanoTime();
        new AnimationTimer() {
            @Override
            public void handle(long now) {
//                double t = (now - startNanoTime) / 1_000_000_000.0;
//                double d = (now - startNanoTime) / 1_150_000_000.0;
//                double nT = (now - startNanoTime) / 1_000_000_000_000_000.0;
//                double nD = (now - startNanoTime) / 1_150_000_000.0;
                if(spin == true){
                    double t = (now - startNanoTime) / 1_000_000_000.0;
                    double d = (now - startNanoTime) / -1_150_00_000.0;
                    earthView.setX(xc - earth.getWidth() / 2 + radius * Math.cos(t));
                    earthView.setY(yc - earth.getWidth() / 2 + radius * Math.sin(t));

                    deathstarView.setX(xc - deathstar.getWidth() / 2 + radiusD * Math.cos(d));
                    deathstarView.setY(yc - deathstar.getWidth() / 2 + radiusD * Math.sin(d));
                }
//                else{
//                    earthView.setX(xc - earth.getWidth() / 2 + radius * Math.cos(nT));
//                    earthView.setY(yc - earth.getWidth() / 2 + radius * Math.sin(nT));
//
//                    deathstarView.setX(xc - deathstar.getWidth() / 2 + radiusD * Math.cos(nD));
//                    deathstarView.setY(yc - deathstar.getWidth() / 2 + radiusD * Math.sin(nD));
//                }
            }
        }.start();        
    }
    ...
    public void createControls(){
        ...
        addButton("Stop/Start", e -> earthControl());
//        addNewHBox();
    }
    @Override
