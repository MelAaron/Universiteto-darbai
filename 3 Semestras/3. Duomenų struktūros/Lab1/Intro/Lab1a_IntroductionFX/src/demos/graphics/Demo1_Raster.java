...
public class Demo1_Raster extends BaseGraphics {
    ...
    void drawSetOfCirclesMano(){ 
        ...
        for(int i=0; i<count; i++){
            ...
            gc.strokeOval(x+dif/2, y, canvasW-x-x, canvasH-y-y);
            gc.strokeOval(x-2*dif, y, canvasW - x - x, canvasH - y - y);
            gc.strokeOval(x+2*dif, y, canvasW - x - x, canvasH - y - y);
            x += dx;
            y += dy;
        }
    ...
    public void createControls(){
        ...
        addButton("LinesMano",     e -> drawSetOfLinesMano());
        addButton("CirclesMano",     e -> drawSetOfCirclesMano());
        addNewHBox();
    }
    @Override
