...
// http://www.java2s.com/Tutorials/Java/Java_Regular_Expression/0070__Java_Regex_Groups.htm    
    ...
    ***************************************************/
     void regexExamples(){                                                       // atkomentuot viena is variantu arba perrasyt koncolei bet vietoi dvieju slash (\\) rasyt tik viena (\)
        //String d ="-";                                                        // atpazina minusa variantas
        //String d = "(\\p{IsLatin}+)";                                         // a variantas
        //String d = "\p{IsLatin}*([ąčęėįšųūžĄČĘĖĮŠŲŪŽ]=?)\p{IsLatin}*";        // b variantas
        //String d ="\\(.*?\\)";                                                  // c variantas
        String d = "/-+/g";
        if(ta1.getText().isEmpty())  // jei tuščia - pateikiame demo variantą
            ta1.appendText(d+"<>pirmas=1;antras>-22:trečias:333(viskas)" + nL);
     ...
     void regexExamples2() {
         ...
         String d = "(\\p{IsLatin}+)"; 
         //String d ="-";  
        if (this.ta1.getText().isEmpty()) {
            this.ta1.appendText("d+<>pirmas=1;antras>-22:trečias:333viskas\n");
        }

        String[] sa = this.readLastLine().split("<>");
        this.ta2.appendText(String.join(" : ", sa) + "\n");
        Pattern pat = Pattern.compile(sa[0]);
        Matcher m = pat.matcher(sa[1]);

        while(m.find()) {
            this.ta2.appendText(m.group() + "\n");
        }
    ...
    public void createControls() {
        ...
        addButton("regex",   action -> regexExamples2());         
    }
    @Override
