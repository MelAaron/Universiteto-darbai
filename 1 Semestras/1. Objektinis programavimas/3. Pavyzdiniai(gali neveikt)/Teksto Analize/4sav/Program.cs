...
namespace _4sav
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //string k;
            //k = Console.ReadLine();
            //Console.WriteLine(k.Substring(1, 4));
            //Console.WriteLine(skyr.IndexOf(k[0], 0));

            //Console.WriteLine("Iveskite eilute: ");
            //string fv;
            //fv = Console.ReadLine();
            //p.Apdoroti(fv, CFr, skyr, vardas, pavarde);
        ...
        void Zodziai(string line, string skyrikliai, string vardas, string pavarde, StringBuilder nauja)
        {
            ...
            while (ind != -1)
            {
                if (skyrikliai.IndexOf(papild[ind - 1]) != -1 && skyrikliai.IndexOf(papild[ind + vardas.Length]) != -1)
                {
                    ...
                    naujausia = line.Remove(skyrikliai.IndexOf(papild[ind - 1], skyrikliai.IndexOf(papild[ind + vardas.Length])));
                    
                }
                ind = papild.IndexOf(vardas, ind + 1);
            }
            Console.WriteLine(naujausia);
            nauja.Append(line.Substring(prad - 1));
        }

        public void Nauja (string line, string skyrikliai, string vardas, string pavarde, StringBuilder nauja)
        {
            string naujausia = ".";
            string papild = " " + line + " ";
            int prad = 1;
