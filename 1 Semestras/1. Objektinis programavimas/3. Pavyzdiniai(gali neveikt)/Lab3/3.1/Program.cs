...
namespace Lab3
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("Agresyvus Kauno sunys: {0}", p.CountAggressive(branches[0].Dogs));
            Console.WriteLine("Agresyvus Vilniaus sunys: {0}", p.CountAggressive(branches[1].Dogs));

            AnimalsContainer kaunasDogs = branches[0].Dogs;
            AnimalsContainer vilniusCats = branches[1].Cats;
            Console.WriteLine("Populiariausia sunu veisle Kaune: {0}", p.GetMostPopularBreed(kaunasDogs));
            Console.WriteLine("Populiariausia kaciu veisle Vilniuje: {0}", p.GetMostPopularBreed(vilniusCats));
            Console.WriteLine();
        ...
        private string GetMostPopularBreed (AnimalsContainer animals)
        {
            ...
            {
                AnimalsContainer filteredAnimals = FilteredByBreed(animals, breeds[i]);
        ...
        private AnimalsContainer FilteredByBreed(AnimalsContainer animals, string breed)
        {
        ...
        private bool ReadAnimalData (string file, Branch[] branches)
        {
            ...
            using (StreamReader reader = new StreamReader(@file))
            {
                ...
                while (null != (line = reader.ReadLine()))
                {
                    ...
                    {
                        case 'D':
                            //atkreipkite demesi - sunys turi papildoma pozymi aggressive
                            //irasom chipID
                            string name = values[1];
                            int chipId = int.Parse(values[2]);
